...
fn run_picker_mode(prefetch_runs: usize) -> Result<()> {
    ...
    let res: Result<()> = loop {
        ...
        terminal.draw(|f| draw_picker_ui(f, &app))?;
...
fn load_compare_app_with_spinner(
    ...
    loop {
        throbber.calc_next();
        terminal.draw(|f| draw_loading_ui(f, label, &mut throbber))?;
...
fn draw_loading_ui(f: &mut Frame, label: &str, throbber_state: &mut ThrobberState) {
...
fn draw_picker_ui(f: &mut Frame, app: &PickerApp) {
